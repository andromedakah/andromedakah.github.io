...
- **Landing search/explore index.** The landing feed has an "Explore" panel (search box +
  quick-filter chips for verticals/companies + a clickable topic mind map). It is generated by
  `python3 scripts/feed_explore.py` (reads `tags.json` + each edition's `ai-radar.md`; rewrites
